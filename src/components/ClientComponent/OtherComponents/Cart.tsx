...
const Cart: React.FC = () => {
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      const result = await orderStore.placeOrder(cartStore.cartItems, userId, paymentMethod, orderNote, totalPrice);
      if (result) {
  ...
  return (
    <div className="w-full md:w-[1180px] m-auto mt-5 pb-10">
      ...
      {cartStore.cart.items.length === 0 ? (
        <Alert variant="outlined" severity="error">Your cart is empty.
        </Alert>
      ) : (
        <div>
          {messageBox ?
            <Alert variant="outlined" severity="success"> Your Order has been Placed
            </Alert>
            :
            <div className="flex gap-x-6 mt-5">
              <div className="gap-4 flex flex-col w-[60%]">
                {cartStore.cartItems.map((item: CartItem, index: number) => {
                  ...
                  return (
                    <div key={index} className="bg-white flex shadow-xl rounded-lg items-center justify-between p-4 h-[100px]">
                        ...
                        <div className="ml-3">
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center">
                              <Button variant="light" onClick={() => decreaseQuantity(item.product_id)}>-</Button>
                              <span className="px-3">{item.quantity}</span>
                              <Button variant="light" onClick={() => increaseQuantity(item.product_id)}>+</Button>
                            </div>
                      ...
                      <button
                        ...
                        onClick={() => cartStore.removeItemFromCart(item.product_id, userStore.user?.id)}
                      >
                        Remove
                      </button>
                    </div>
