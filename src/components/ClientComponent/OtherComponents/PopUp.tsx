...
interface PopProps {
  ...
  handleAddOnChange: (name: string) => void;
  handleFunOptionChange: (name: string) => void;
  handleQuantityChange: (name: string, value: number, type: 'addOn' | 'funOption') => void;
  setQuantity: (quantity: number) => void;
  calculateTotal: () => number;
}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <button
        ...
        <div className="flex">
          ...
          />
          <div className="ml-3">
            <Text themeDivClasses="text-lg font-semibold" content={selectedMenuItem.name} />
            <Text themeDivClasses="text-sm" content={selectedMenuItem.description} />
          </div>
        </div>
        <div className="my-4">
          <Text themeDivClasses="font-bold text-xl mb-2" content="Quantity" />
        ...
        <div className="my-4">
          <Text themeDivClasses="font-bold text-xl mb-2" content="Add-Ons" />
          {addOns&&Object.keys(addOns).map((key) => (
              ...
              {addOns[key].selected && (
                <div className="flex items-center ml-2">
                  <button
                    ...
                    onClick={() =>
                      handleQuantityChange(key, addOns[key].quantity - 1 >= 1 ? addOns[key].quantity - 1 : 1, "addOn")
                    }
                  >
                    -
                  </button>
                  <Text themeDivClasses="mx-2" content={`${addOns[key].quantity}`} />
                  <button
                    className="px-2 py-1 bg-gray-200 rounded-r"
                    onClick={() => handleQuantityChange(key, addOns[key].quantity + 1, "addOn")}
                  >
                    +
                  </button>
                </div>
              )}
              <Text themeDivClasses="ml-auto" content={`$${addOns[key].price}`} />
            </div>
          ))}
        </div>
        <div className="my-4">
          <Text themeDivClasses="font-bold text-xl mb-2" content="Fun Options" />
          {funOptions&& Object.keys(funOptions).map((key) => (
            <div key={key} className="flex items-center mb-2">
              <input
                type="checkbox"
              ...
              {funOptions[key].selected && (
                <div className="flex items-center ml-2">
                  <button
                    className="px-2 py-1 bg-gray-200 rounded-l"
                    onClick={() =>
                      handleQuantityChange(key, funOptions[key].quantity - 1 >= 1 ? funOptions[key].quantity - 1 : 1, "funOption")
                    }
                  >
                    -
                  </button>
                  <Text themeDivClasses="mx-2" content={`${funOptions[key].quantity}`} />
                  <button
                    className="px-2 py-1 bg-gray-200 rounded-r"
                    onClick={() => handleQuantityChange(key, funOptions[key].quantity + 1, "funOption")}
                  >
                    +
                  </button>
                </div>
              )}
              <Text themeDivClasses="ml-auto" content={`$${funOptions[key].price}`} />
            </div>
          ))}
        </div>
        <div className="my-4">
          <Text themeDivClasses="font-bold text-xl mb-2" content={`Total: $${calculateTotal().toFixed(2)}`} />
        </div>
