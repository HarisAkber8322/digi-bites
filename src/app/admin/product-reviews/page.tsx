    ...
    useEffect(() => {
        const fetchUserNames = async () => {
            ...
            const userNamePromises = uniqueUserIds.map(async id => {
                ...
                return { id, name: user ? `${user.fname} ${user.lname}` : 'Unknown User' };
            });

            const userNamesArray = await Promise.all(userNamePromises);
            const userNamesMap = userNamesArray.reduce((map, user) => {
                                            ...
                                            <tr key={index} className="hover:bg-gray-100 dark:hover:bg-gray-700 border-b border-b-lightGray">
                                                ...
                                                <div className="flex justify-center flex-col gap-1 items-center w-full">
                                                    ...
                                                    {product.ratings.map((item, index) => (
                                                        <span key={index}>{userNames[item.user_id] || 'Unknown User'} <br /></span>
                                                    ))}
                                                    </div>
                                                </td>
                                                <td className="px-4 py-2 text-center whitespace-nowrap text-xs text-blue-500">
                                                    <div className="flex justify-center flex-col gap-1 items-center w-full">
                                                        {product.ratings.map((item, index) => (
                                                            <div>
                                                                <div key={index} className="flex justify-center items-center bg-blue-100 rounded-md w-8 h-5">
                                                                    <span>{item.rating + ' '}</span> <FaStar className="inline ml-1 text-blue-500" />
                                                                </div>
                                                            </div>
                                                        ))}
                                                    </div>
